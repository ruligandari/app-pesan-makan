...
class DetailKeranjang extends StatefulWidget {
  ...
  String nama_produk;
  String kuantitasProduk;
  String id_keranjang;
  DetailKeranjang(
      this.id, this.nama_produk, this.kuantitasProduk, this.id_keranjang);

  @override
  State<DetailKeranjang> createState() => _DetailState();
}

class _DetailState extends State<DetailKeranjang> {
  ...
  Future updateKeranjang(id) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String? token = prefs.getString('token');
    String? id = prefs.getString('id');
    var url = Uri.parse("$endpoints/api/keranjang/update");
    var header = {'Authorization': 'Bearer $token'};
    try {
      final response = await http.post(url, headers: header, body: {
        "id_keranjang": widget.id_keranjang,
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          ...
          content: Text('Berhasil Update Keranjang'),
        ...
        floatingActionButton: Container(
          ...
          child: Row(
            ...
            children: [
              InkWell(
                onTap: () {
                  updateKeranjang(widget.id_keranjang);
                },
                child: Container(
                  height: 50,
                  width: 300,
